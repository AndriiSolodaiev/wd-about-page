...
const userPhoneVacancies = document.querySelector(
  '.form-vacancies [type="tel"]'
);
const userNameVacation = document.querySelector("#vacation-form-input-name");
const messageNameVacancies = document.querySelector(
  ".input-vacancies-message-name"
);
...
callbackForm.addEventListener("submit", (event) => {
  ...
  let hasError = false;
  const requiredFieldsCount = event.target.querySelectorAll(
    '[data-required="true"]'
  ).length;
  let validFields = 0;
    ...
    send(
      ...
      function () {
        ...
        if (feedbackForm.closest(".sideform-active") !== null) {
          feedbackForm
            .closest(".sideform-active")
            .classList.remove("sideform-active");
        }
...
vacanciesForm
  ...
  .addEventListener("change", (evt) => {
    if (evt.target.files.length === 0) {
      vacanciesForm.querySelector(".field__file-wrapper").innerText =
        "Выберите файл";
    } else {
      vacanciesForm.querySelector(".field__file-wrapper").innerText =
...
vacanciesForm.addEventListener("submit", (event) => {
  ...
  if (
    !userPhoneVacancies.value.trim() ||
    !isPhoneValid(userPhoneVacancies.value)
  ) {
...
closeAccButtons.forEach((el) =>
  el.addEventListener("click", (e) => {
    ...
    if (!item) return;
    const headerHeight =
      document.getElementsByTagName("header")[0].offsetHeight * -1;
    console.log(headerHeight);
    window.locoScroll.scrollTo(item, {
      offset: headerHeight,
    });
...
const addNumberToTitle = () => {
  ...
  [...arrProjectTitle].map((elem, ind) => {
    const numberProject = `<span class="project-title-number">${pad(
      ind + 1
    )}.</span>`;
...
const menuItemHandler = () => {
  ...
  const filterArrNamesPeges = arrNamesPeges.filter((name) =>
